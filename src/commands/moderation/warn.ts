...
export default {
    ...
    async execute(client:SynicClient, message:Message, args:ReadonlyArray<string>) {
        ...
        if (!message.guild?.me?.hasPermission("MANAGE_MESSAGES")) return simpleEmbed(message, responses.moderation.no_bot_permission, colors.error);
        const warnUser = message.guild?.member(message.mentions.users.first() || message.guild?.members.cache.get(args[0])!)
        if (!warnUser) return simpleEmbed(message, responses.moderation.no_user_provided + "warn!", colors.error);
        if (warnUser.hasPermission("MANAGE_MESSAGES")) return simpleEmbed(message, responses.moderation.warn.could_not_warn_user, colors.error);
        if (warnUser.id === message.author.id) return simpleEmbed(message, responses.moderation.warn.cannot_warn_self, colors.error);
